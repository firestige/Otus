...
make build-all
# 输出在 dist/ 目录：
#   dist/otus-linux-amd64
#   dist/otus-linux-arm64
```
...
# curl -LO https://github.com/firestige/otus/releases/download/v1.0.0/otus-linux-amd64
```
...
```bash
# 复制到系统路径
sudo install -m 755 otus-linux-amd64 /usr/local/bin/otus
...
# 数据目录
sudo mkdir -p /var/lib/otus

# 日志目录
sudo mkdir -p /var/log/otus
```
...
2. **云服务器限制**:
   ...
   ```bash
   # 阿里云示例：配置流量镜像
   # 在控制台创建流量镜像会话，将源实例流量镜像到 Otus 实例
   ```
      ...
      - name: otus
        ...
        livenessProbe:
          exec:
            ...
            - /bin/sh
            - -c
            - "test -e /tmp/otus.pid && kill -0 $(cat /tmp/otus.pid)"
...
# 2. 创建 ConfigMap
kubectl apply -f configmap-otus.yaml

# 3. 部署 DaemonSet
kubectl apply -f daemonset-otus.yaml

# 4. 查看 Pod 状态
kubectl get pods -n monitoring -l app=otus -o wide

# 5. 查看日志
kubectl logs -n monitoring -l app=otus --tail=100 -f

# 6. 验证指标
kubectl exec -n monitoring -it $(kubectl get pod -n monitoring -l app=otus -o name | head -1) -- curl localhost:9091/metrics
```

### Prometheus 集成

```yaml
# servicemonitor-otus.yaml
...
# 应用配置
sudo sysctl -p /etc/sysctl.d/99-otus.conf
```
